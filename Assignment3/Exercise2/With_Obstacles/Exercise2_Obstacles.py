# -*- coding: utf-8 -*-
"""
Created on Thu Dec  6 23:52:11 2018
...
#obs = []
obs = [[4,0],[4,4.7],[4,8],[6,2],[7,6],[6,10],[8,4],[8,12]] # 8 obstacles
#obs = [[4,0],[4,4.7],[4,8],[6,2],[7,6],[6,10],[8,4],[8,12], [8.1,5.6]] # 9 obstacles
...
for i in range(1, int(t/dt)):
    ...
    posx_acc = K_f*(np.matmul(inc, zx_ref) - np.matmul(L, pos_x[i-1])) - D_f*np.matmul(L, vel_x) + (K_t*min(diff_x(x_des, pos_x[i-1]), 1.)) - D_t*np.array(vel_x) + ff_x 
    posy_acc = K_f*np.matmul(inc, zy_ref) - K_f*np.matmul(L, pos_y[i-1]) - D_f*np.matmul(L, vel_y) + (K_t*min(diff_y(y_des, pos_y[i-1]), 1.)) - D_t*np.array(vel_y) + ff_y
...
for i in range(4):
    ...
    if len(obs) == 8:
        plt.title('Behavior of the robots with 8 obstacles')
    if len(obs) == 9:
        plt.title('Behavior of the robots with 9 obstacles')
    ...
    if len(obs) == 8:
        plt.savefig('xy_position_8')
    if len(obs) == 9:
        plt.savefig('xy_position_9')
plt.show()
...
#obstacles = np.array([[4,0],[4,4.7],[4,8],[6,2],[7,6],[6,10],[8,4],[8,12], [8.1, 5.6]])
obstacles = obs
if len(obs) == 8:
    make_animation_obstacles(plotx, E, obstacles, inter=100, display=True, xl=(-1, 12), yl=(-1, 12)).save("8_obstacles.mp4")
if len(obs) == 9:
    make_animation_obstacles(plotx, E, obstacles, inter=100, display=True, xl=(-1, 12), yl=(-1, 12)).save("9_obstacles.mp4")